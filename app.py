# app.py - COMPLETE VERSION WITH ALL TABLES & ROUTES
import os
...
def order_history():
    try:
        ...
        # Debug info
        if orders_list:
            print(f"🔍 [ORDER_HISTORY] First order has {len(orders_list[0]['items'])} items")
            if orders_list[0]['items']:
                print(f"🔍 [ORDER_HISTORY] First item photo: {'✅' if orders_list[0]['items'][0].get('photo') else '❌'}")
